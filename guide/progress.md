...
Takes the weather-joined dataset and produces a lean, model-ready parquet. Drops NaN rows and DR hours, caps extreme outliers, encodes time features as cyclical sin/cos pairs, builds per-station lag features (1h, 2h, 3h, 24h, 168h), rolling statistics (6h, 12h, 24h, 7-day mean/std), load momentum (load_change_1h), load ratio vs baseline (load_ratio_24h), hour×weekend interaction, cold weather flag, label-encodes categoricals, normalizes station attributes, and drops all unused columns. Output is ready for modeling.
